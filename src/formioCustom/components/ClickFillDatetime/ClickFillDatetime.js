...
import editForm from './ClickFillDatetime.form';

export default class ClickFillDatetimeComponent extends Input {
    ...
    static get builderInfo() {
        return {
            ...
            weight: 40,
            schema: ClickFillDatetimeComponent.schema()
        };
    }
    ...
    attach(element) {
        //console.log('kl_', element);
        const inputGroup = element.querySelector('.input-group');

        inputGroup && inputGroup.addEventListener('click', () => {
            const input = inputGroup.querySelector('.input');

            if (!input.value && this.component.onClickSetToday && !this.component.disabled) {
                input.value = moment().format(this.component.format.toUpperCase());
            }
        });
    ...
    get defaultSchema() {
        return ClickFillDatetimeComponent.schema();
    }
