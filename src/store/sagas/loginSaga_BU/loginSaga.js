    ...
    while (true) {
        ...
        try {
            ...
            if (data.respCode === 0) {
                ...
                //config call
                const [encounterTypes, rooms, clinicConfig, listConfig, quotaConfig, serviceSessionsConfig, deleteReasons, specialRqstTypes, clnDefaultRmConfig] = yield all([
                    ...
                    // call(maskAxios.get, `/doc-upload/docUpload/outDocumentTypes/${data.data.service_cd}`)
                ]);
                ...
                if (data.data.service_cd == 'DTS') {
                    ...
                    yield put({ type: dtsPreloadDataActionType.GET_DTP_MAND_ETYPE_CNC_CODE, params: 'DTP_MANDATORY_ENCOUNTER_TYPE'});
                ...
                //     yield put({ type: commonType.SET_OUT_DOCUMENT_TYPES, data: outDocumentTypesList });
                // } else {
                //     yield put({
...
function* doLoginOut() {
    while (true) {
        ...
        try {
            ...
            if (loginWithSam) {
                yield call(axios.get, '/user/logout/' + loginName);
            }else{
    ...
    yield alsTakeEvery(types.PRE_LOAD_DATA, function* (action) {
        try {
            ...
            if (clientIps.data.respCode === 0) {
                ...
                const ipInfo = { ipAddr: clientData.ipAddr, pcName: clientData.pcName };
                ...
                if (serviceAndClinicGp) {
                    ...
                    if (clientServiceList.length > 0 && clientSiteList.length > 0) {
                        const default_svcCd = clientServiceList[0]['svcCd'];
                        const default_site = clientSiteList.find(item => item.svcCd === default_svcCd);
                        ...
                        if (lastLoginServiceClinc) {
                            ...
                            const clinicIndex = clientSiteList.filter(item => item.svcCd === lastLoginServiceClinc.svcCd).findIndex(item => parseInt(item.siteId) === parseInt(lastLoginServiceClinc.siteId)); if (serviceIndex !== -1 && clinicIndex !== -1) {
