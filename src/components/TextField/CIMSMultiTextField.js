...
class CIMSMultiTextField extends Component {
    ...
    handleOnChange = (e) => {
        ...
        //word per rows
        if (this.props.wordMaxWidth && parseInt(this.props.wordMaxWidth) > 0) {
    ...
    render() {
        /* eslint-disable */
        const { classes, onBlur, onChange, InputProps,//NOSONAR
            inputProps, upperCase, value, calActualLength,//NOSONAR
            type, onlyOneSpace, wordMaxWidth, ...rest//NOSONAR
        } = this.props;
        /* eslint-enable */
        let custom_inputProps = inputProps || {};
        if (upperCase) {
            custom_inputProps.style = Object.assign({}, custom_inputProps.style, { textTransform: 'uppercase' });
        }
...
CIMSMultiTextField.defaultProps = {
    type: 'text',
    trim: 'all'
};

CIMSMultiTextField.propTypes = {
    /**
...
export default withStyles(styles)(CIMSMultiTextField);