...
import CIMSSelect, { CreatableSelect } from '../Select/CIMSSelect';
...
class SelectComponent extends ValidatorComponent {
    ...
    selectElement(props) {
        ...
        const { onChange, selectClassName, isCreatable, ...rest } = props;//NOSONAR
        const { isValid } = this.state;
        const Com = isCreatable ? CreatableSelect : CIMSSelect;
        return (
            <Com
...
const SelectFieldValidator = React.forwardRef((props, ref) => {
    const { validators, errorMessages, ...rest } = props;
    const { isDisabled } = props;
    const addNullOption = !isDisabled && validators && validators.findIndex(x => x === ValidatorEnum.required) > -1 ? false : true;
    return (
        <SelectComponent
