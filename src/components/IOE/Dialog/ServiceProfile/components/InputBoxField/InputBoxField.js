...
class InputBoxField extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    let val = '';
    let disabledFlag = !isNull(valObj)&&!isUndefined(valObj)?!valObj.isChecked:true;
    switch (itemValType) {
  ...
  handleChanged = event => {
    const { id,middlewareObject,categoryType,updateStateWithoutStatus,itemValType,infoEditMode=false } = this.props;
    let valMap = middlewareObject[`${categoryType}ValMap`];
  ...
  handleBlur = () => {
    const { id,middlewareObject,categoryType,updateStateWithoutStatus,itemValType,sideEffect,infoEditMode=false } = this.props;
    let valMap = middlewareObject[`${categoryType}ValMap`];
    let valObj = valMap.get(id);
    let val = '';
    switch (itemValType) {
  ...
  render() {
    const { classes, id='', maxLength, nullAble='Y' } = this.props;
