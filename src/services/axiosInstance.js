...
const getFunctionName = (reduxState, functionCode) => {
  ...
  if(reduxState.login && reduxState.login.accessRights && functionCode){
    if(reduxState.mainFrame && reduxState.mainFrame.tabsActiveKey){// get active tab label
      let activeLabel = '';
      let activeKey = reduxState.mainFrame.tabsActiveKey;
      if(activeKey === 'patientSpecFunction'){
        let subActivekey =  reduxState.mainFrame.subTabsActiveKey;
        activeLabel = reduxState.mainFrame.subTabs.find(a => a.name === subActivekey)?.label || '';
      }else{
        activeLabel = reduxState.mainFrame.tabs.find(a => a.name === activeKey)?.label || '';
      }

      if(activeLabel === null || activeLabel === undefined || activeLabel === ''){
...
const handleError = (err) => {
  ...
  if((err.response&&err.response.config.url.indexOf('/als') === -1) || (err.response&&err.response.config.url.indexOf('/refreshTokenWithSam') >= 0 && err.response.status !== 401) ){
