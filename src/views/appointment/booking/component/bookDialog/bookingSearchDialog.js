...
class BookingSearchDialog extends React.Component {
    constructor(props) {
        super(props);
        ...
        const { bookingTimeSlot, bookSqueezeInTimeSlotData, bookConfirmSelected, bookingData, isMultiple } = this.props;
        this.state.copyBookingTimeSlot = bookSqueezeInTimeSlotData ? _.cloneDeep(bookSqueezeInTimeSlotData) : _.cloneDeep(bookingTimeSlot);
    ...
    onChangeSlotDate = (date, index) => {
        ...
        copyBookingTimeSlot[index]['dateDiff'] = AppointmentUtil.getDateDiff(startDate, date, bookingData.multipleInterval, bookingData.multipleIntervalUnit);
        if (copyBookingTimeSlot.length > index + 1) {
            copyBookingTimeSlot[index + 1]['dateDiff'] = AppointmentUtil.getDateDiff(date, copyBookingTimeSlot[index + 1]['slotDate'], bookingData.multipleInterval, bookingData.multipleIntervalUnit);
        }
    ...
    checkStartTime = (timeSlot) => {
        const { bookingData, timeSlotList } = this.props;
    ...
    onChangeStartTime = (e, index, timeSlot) => {
        ...
        const { bookingData, timeSlotList, serviceCd } = this.props;
        if (!copyBookingTimeSlot) return;
        if (this.checkStartTime(timeSlot)) {
            const bookingUnit = parseInt(bookingData.bookingUnit);
            if (bookingUnit > 1) {
                const selectedIndex = timeSlotList.findIndex(x => x.tmsltId === timeSlot.tmsltId);
                const selectedList = timeSlotList.slice(selectedIndex, selectedIndex + bookingUnit);
                ...
                if (serviceCd === 'EHS') {
                    copyBookingTimeSlot[index]['list'] = [{
                        ...
                        encntrTypeIdDefault: timeSlot.encntrTypeIdDefault,
                        encntrTypeIdDefaultDesc: timeSlot.encntrTypeIdDefaultDesc
                    }];
                } else {
                    copyBookingTimeSlot[index]['list'] = [{
                        startTime: timeSlot.stime,
    ...
    render() {
        const { classes, open, id, timeSlotList, bookingData, serviceCd } = this.props;
                                    ...
                                    >
                                        {
                                            matrixCols.map(item => {
                                                return (
                                                    <Grid key={item} item container direction="column" style={{ width: '22%', marginRight: serviceCd === 'EHS' ? 95 : 8 }}>
                                                        {
                                                            timeSlotList.slice(item * 10, (item + 1) * 10).map((i, index) => {
                                                                let labelStr = '';
                                                                if (serviceCd === 'EHS') {
                                                                    labelStr = `${moment(i.stime, Enum.TIME_FORMAT_24_HOUR_CLOCK).format(Enum.TIME_FORMAT_24_HOUR_CLOCK)} (${AppointmentUtil.getQuotaDisplayContent(i.booked || 0, i.total || 0, isShowAvailable)})  ${i.encntrTypeIdDefaultDesc || ''}`;
                                                                } else {
                                                                    labelStr = `${moment(i.stime, Enum.TIME_FORMAT_24_HOUR_CLOCK).format(Enum.TIME_FORMAT_24_HOUR_CLOCK)} (${AppointmentUtil.getQuotaDisplayContent(i.booked || 0, i.total || 0, isShowAvailable)})`;
                                                                }
                                                                    ...
                                                                    >
                                                                        <FormControlLabel
                                                                            ...
                                                                            style={{
                                                                                ...
                                                                                minWidth: serviceCd === 'EHS' ? 220 : '120px',
                                                                            ...
                                                                            checked={copyBookingTimeSlot[tabValue].list[0]['slotId'] === i.tmsltId ? true : false}
                        ...
                        [
                            {
                                ...
                                onClick: () => {
                                    ...
                                    this.props.handleSave(this.state.copyBookingTimeSlot);
                                },
...
export default (connect(mapStateToProps, dispatchToProps)(withStyles(styles)(BookingSearchDialog)));