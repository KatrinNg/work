...
class SpecimenContainer extends Component {
  ...
  UNSAFE_componentWillMount(){
    EventEmitter.on('ix_request_edit_dialog_specimen_scroll', this.handleScroll);
  }

  componentWillUnmount(){
    EventEmitter.remove('ix_request_edit_dialog_specimen_scroll', this.handleScroll);
  }

  handleScroll = (payload={}) => {
    let { labId, formId, itemId } = payload;
    let anchorElement = document.getElementById(`edit_specimen_${labId}_${formId}_${itemId}`);
  ...
  generateGroupItems = items => {
    ...
    if (items.length>0) {
      items.forEach(item => {
        ...
        itemElements.push(
          <div key={`${selectedLabId}_${selectedFormId}_${item.codeIoeFormItemId}`} id={`edit_specimen_${selectedLabId}_${selectedFormId}_${item.codeIoeFormItemId}`} className={classes.itemWrapperDiv}>
