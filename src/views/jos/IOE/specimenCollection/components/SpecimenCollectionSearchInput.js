...
import {styles} from './SpecimenCollectionSearchStyle';
import {trim,debounce} from 'lodash';
import { InfiniteLoader, List } from 'react-virtualized';

class SpecimenCollectionSearchInput extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: false,
  ...
  closeSearchData = (e) => {
    const{insertSpecimenCollectionLog}=this.props;
    this.setState({
      open: false,
      value: '',
      keepDataSelected: false,
    ...
    this.debounceChange('');
    insertSpecimenCollectionLog&&insertSpecimenCollectionLog('Action: Click \'X\' button to clear searching text in \'Search by Requested Date / Test / Form Name\'','');
  }
    ...
    if (!!item) {
      ...
      return (
        <MenuItem
            ...
            style={style}
        >
          <Typography className={classes.mr15}
              component="span"
              noWrap
              variant="caption"
          >
  ...
  render() {
    const { classes,id='',inputPlaceHolder='',totalNums=9000,pageSize=20 } = this.props;
    let {
      open,
      value,
      openSearchProgress,
        ...
        >
          <Paper className={classes.inputPaper}>
            ...
            <IconButton
                ...
                style={{display: value.length === 0 ?'none':'initial',float:'right'}}
            >
              {openSearchProgress ? (
...
SpecimenCollectionSearchInput.propTypes = {
  onChange: PropTypes.func.isRequired,
  onSelectItem: PropTypes.func.isRequired
};

SpecimenCollectionSearchInput.defaultProps = {
  onChange: () => {},
  onSelectItem: () => {}
};

export default withStyles(styles)(SpecimenCollectionSearchInput);
