...
import { styles } from './SubstanceAbuseTableStyle';
...
class SubstanceAbuseTable extends Component {
  constructor(props){
    super(props);
  ...
  handleRowAdd = () => {
    ...
    updateState&&updateState({
      substanceAbuseHistoryList: _.concat(dataList,tempObj),
      valMap
    });
  ...
  handleRowDelete = () => {
    ...
    if (dataList.length>0) {
      ...
      if (tempArray.length>0) {
        ...
        updateState&&updateState({
          substanceAbuseHistoryList: dataList,
          valMap
        });
      }
    }
  ...
  generateTableContent = () => {
    ...
    elements = dataList.map(item => {
      ...
      let typeFieldProps = {
        ...
        options: dropdownOption?dropdownOption.typeOptionMap.get(socialHistoryType.substanceAbuseId):[],
        ...commonProps
      };
      ...
      return (
        <TableRow
            key={`social_history_substance_abuse_table_row_${item.socialHistoryDetailsId}`}
    ...
    return (
      ...
      >
        <ValidatorForm onSubmit={()=>{}}>
          <Table id="social_history_substance_abuse_table" ref={this.tableRef} >
            <TableHead>
              {viewMode?(
...
export default withStyles(styles)(SubstanceAbuseTable);
