...
class PdfPreviewDialog extends React.Component {
    ...
    print = () => {
        ...
        if (previewData) {
            this.props.auditAction('Click Print in preview pdf/image dialog', null, null, false, 'clinical-doc');
            try {
                const blob = moeUtilities.b64toBlob(previewData, `${ (fileExtension === 'pdf' ? 'application' : 'image') }/${ fileExtension }`);
                const blobUrl = URL.createObjectURL(blob);
                const iframe = document.createElement('iframe');
                iframe.id = 'clinicalDocIframe';
                iframe.src = blobUrl;
...
/*                setTimeout(() => {
    ...
    render() {
        const {id, previewData, previewShow, classes, isPdf, fileExtension} = this.props;
                    ...
                    <DialogActions>
                        {isPdf ?
                            <Grid container style={{dispaly: 'flex'}} direction={'row'} alignItems="center">
                                <IconButton id={id + 'FirstPageIconButton'} disabled={this.state.pageNumber <= 1}
                                    onClick={this.goToFirstPage}
                                >
                                    <FirstPage>First page</FirstPage>
                                </IconButton>
                                <IconButton id={id + 'PrevPageIconButton'} disabled={this.state.pageNumber <= 1}
                                ...
                                <IconButton id={id + 'NextPageIconButton'}
                                    disabled={this.state.pageNumber >= this.state.numPages}
                                    onClick={this.goToNextPage}
                                >
                                    <ChevronRight>Next page</ChevronRight>
                                </IconButton>
                                <IconButton id={id + 'LastPageIconButton'}
                                    disabled={this.state.pageNumber >= this.state.numPages}
                                    onClick={this.goToLastPage}
                                >
                                    <LastPage>Last page</LastPage>
                                </IconButton>
                                <IconButton id={id + 'ZoomInIconButton'}
                                    disabled={this.state.scale <= this.state.minScale}
                                    onClick={this.onZoonOutHandler}
                                >
                                    <ZoomOut>Zoom in</ZoomOut>
                                </IconButton>
                                <IconButton id={id + 'ZoomOutIconButton'}
                                    disabled={this.state.scale >= this.state.maxScale}
                                    onClick={this.onZoonInHandler}
                                >
                            ...
                            >
                                {isPdf ?
                                    <Document
                                        ...
                                        file={moeUtilities.b64toBlob(previewData, 'application/pdf')}
                                    ...
                                    :
                                    <img id="previewForImage"
                                        src={'data:image/' + fileExtension + ';base64,' + previewData}
                                    />}
