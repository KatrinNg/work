...
import { Grid, Button, Table, TableRow, Typography, TableCell, TableHead, TextField, TableBody, Checkbox } from '@material-ui/core';
...
class EditDiagnosisTemplate extends Component {
  ...
  handleClickUp = () => {
    ...
    if(params){
      ...
      name=`[Problem Template Group Maintenance Dialog] Action: Click 'Up' button ${typeNum} Template ID: ${params.templateId}; Code Term ID: ${params.codeTermId}; Problem Name: ${this.state.groupName})`;
      this.clickUp();
    } else{
      name='[Problem Template Group Maintenance Dialog] Action: Click \'Up\' button (No record is selected; Sequence Number: null; Template ID: null; Code Term ID: null; Problem Name: null)';
      let payload = { msgCode:DIAGNOSIS_TEMPLATE_CODE.IS_UP_PROBLEM_TEMPLATE_TIP};
      this.props.openCommonMessage(payload);
    }
    insertDiagnosisLog && insertDiagnosisLog(name, '');
  };
  ...
  handleClickDown = () => {
    ...
    if(params){
      let typeNum = params.templateId === 0 ? '(New record is selected;' : `(Sequence Number: ${params.displaySeq};`;
      name = `[Problem Template Group Maintenance Dialog] Action: Click 'Down' button (${typeNum} Template ID: ${params.templateId}; Code Term ID: ${params.codeTermId}; Problem Name: ${this.state.groupName})`;
      this.clickDown();
    }
    else{
      name = '[Problem Template Group Maintenance Dialog] Action: Click \'Down\' button (No record is selected; Sequence Number: null; Template ID: null; Code Term ID: null; Problem Name: null)';
  ...
  handleClickOpen = () => {
    let { insertDiagnosisLog } = this.props;
    let params = this.state.selectRowObj;
    if (params) {
      let payload = {
        ...
        btnActions: {
          ...
          btn1Click: () => {
            ...
            let name = `[Problem Template Group Maintenance Dialog] Action: Click 'Delete' button ${typeNum} Code Term ID: ${params.codeTermId}; Problem Name: ${this.state.groupName})`;
            insertDiagnosisLog && insertDiagnosisLog(name, '');
      ...
      let name = '[Problem Template Group Maintenance Dialog] Action: Click \'Delete\' button (No record is selected; Template ID: null; Code Term ID: null; Problem Name: null)';
      insertDiagnosisLog && insertDiagnosisLog(name, '');
    ...
    if (isEdit) {
      let payload = {
        ...
        btnActions: {
          // Yes
          btn1Click: () => {
            insertDiagnosisLog && insertDiagnosisLog('[Problem Template Group Maintenance Dialog] Action: Click \'Cancel\' button', '');
  ...
  recordListSave = () => {
    //Real List
    const { insertDiagnosisLog, commonMessageList } = this.props;
    let List = [];
      ...
      if (Flag) {
        ...
        this.props.saveEditTemplateList({
          ...
          callback: (result) => {
            ...
            insertDiagnosisLog && insertDiagnosisLog('[Problem Template Group Maintenance Dialog] Action: Click \'Save\' button', '/diagnosis/diagnosisTemplate/');
          }
        });
      }
    }
  ...
  handleClickAdd = () => {
    ...
    insertDiagnosisLog && insertDiagnosisLog('[Problem Template Group Maintenance Dialog] Action: Click \'Add\' button', '');
    this.setState({
      addFlag: true,
      isEdit: true
    });
  ...
  handleSearch = (e) => {
    ...
    let checkbox = this.state.problemLocalTermChecked ? 'Yes' : 'No';
    let name = `[Problem Template Group Maintenance Dialog] Action: Search searching word in 'Problem Name' (Local terms: ${checkbox})`;
    ...
    insertDiagnosisLog && insertDiagnosisLog(name, '/diagnosis/codeList/codeDxpxDiagnosisType/');
  };
  ...
  render() {
    ...
    return (
      <EditTemplateDialog
          ...
          dialogTitle="Problem Template Group Maintenance"
          open={open}
          ...
          <ValidatorForm id="bookingCalendarForm" onKeyDown={(e) => this.handleKeyDown(e)} onSubmit={this.recordListSave} ref="form">
            <Grid alignItems="center" container style={{ marginTop: 10 }}>
              ...
              <Grid item xs={8}>
                ...
                {nameErrorFlag ? (
                  <span id="grounName" className={classes.validation_span}>
            ...
            <Typography component="div" style={{ marginTop: 0, marginBottom: -15 }}>
              <Button onClick={this.handleClickAdd} id="btn_editProblemTemplate_add" style={{ textTransform: 'none' }}>
                <AddCircle color="primary" />
                <span className={classes.font_color}>Add</span>
              </Button>

              <Button onClick={this.handleClickOpen} id="btn_editProblemTemplate_delete" style={{ textTransform: 'none' }}>
                <RemoveCircle color="primary" />
                <span className={classes.font_color}>Delete</span>
              </Button>

              <Button onClick={this.handleClickUp} id="btn_editProblemTemplate_up" style={{ textTransform: 'none' }}>
                <ArrowUpwardOutlined color="primary" />
                <span className={classes.font_color}>Up</span>
              </Button>

              <Button onClick={this.handleClickDown} id="btn_editProblemTemplate_down" style={{ textTransform: 'none' }}>
            ...
            <Paper className={classes.paperTable}>
              <Table id="problem_template_dialog">
                <TableHead>
                    ...
                    <TableCell className={classes.table_header} padding={'none'} style={{ width: '10%' }}>
                      <Grid container style={{ minWidth: 420 }}>
                        <Grid container item xs={8}>
                          <label className={classes.label}>Problem&nbsp;Name</label>
                        </Grid>
                        <Grid container item xs={4}>
                          ...
                          <div className={classes.floatLeft}>
                            <Checkbox
                                ...
                                onChange={(event) => {
                                  this.handleProblemLocalTermChange(event);
                                }}
                            />
                          </div>
                        </Grid>
                        ...
                        >
                          ...
                          {item.diagnosisName.trim() === '' && !this.state.saveFlag ? (
                            <Typography component="div" style={{ paddingLeft: 8 }}>
                              <span className={classes.validation} id="span_editProblemTemplate_problemName_validation">
                                This field is required.
                              </span>
                            </Typography>
                          ) : null}
                      ...
                      <TableCell className={classes.table_cell} padding={'none'} style={{ width: '24%' }}>
                        ...
                        >
                          ...
                          {item.diagnosisDisplayName.trim() === '' && !this.state.saveFlag ? (
                            <Typography component="div" style={{ paddingLeft: 8 }}>
                              <span className={classes.validation} id="span_editProblemTemplate_displayName_validation">
                                This field is required.
                              </span>
                            </Typography>
                          ) : null}
